...
def shuffle(cards):
    ...
    first_half=cards[:half_length]
    second_half=cards[half_length:]
    result=[]
...
def trade(first, second):
    ...
    for i in range(counter):
        if sum(first[:f])==sum(second[:s]):
            storage=first[:f]
            first[:f]=second[:s]
            second[:s]=storage
            return 'Deal!'
        elif sum(first[:f])<sum(second[:s]):
            f+=1
        elif sum(first[:f])>sum(second[:s]):
            s+=1
    return 'No deal!'
...
def has_prefix(s, prefix):
    ...
    if prefix==empty:
        return True
    while first(s)==first(prefix):
        s=rest(s)
        prefix=rest(prefix)
        if s==empty or prefix==empty:
            break
    if prefix==empty:
        return True
    return False
...
def has_61A_gene(dna):
    ...
    """
    "*** YOUR CODE HERE ***"
    dna_gene = link('C',link('A',link('T', link('C', link('A', link('T', empty))))))
    return has_sublist(dna,dna_gene)
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    incorrect_pass=[]
    def withdraw_with_pass(amount,passw):
        nonlocal incorrect_pass
        nonlocal balance
        if len(incorrect_pass)>=3:
            return 'Your account is locked. Attempts: %s' %incorrect_pass
        elif passw!=password:
            incorrect_pass+=[passw]
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    return_val=withdraw(0,old_password)
    if type(return_val)==str:
        return return_val
    else:
        def changed_pass_withdraw(amount, passw):
            if passw==new_password or passw== old_password:
                return withdraw(amount,old_password)
            else:
                return withdraw(amount, passw)
        return changed_pass_withdraw
