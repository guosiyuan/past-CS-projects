...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        new_value=scheme_eval(procedure.body, new_frame)
        return new_value
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        return do_define_form(Pair(target.first, Pair(Pair('lambda', Pair(target.second, vals.second)), nil)), env)
...
def do_let_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    for i, clause in enumerate(bindings):
        check_form(clause, 2, 2)

        if not scheme_symbolp(clause[0]):
            raise SchemeError("Binding name {0} not valid".format(str(clause[0])))
...
def do_if_form(vals, env):
    ...
    statement=scheme_eval(vals[0],env)
    if(scheme_true(statement)):
        return vals[1]
    else:
        if(len(vals)==2):
            return okay
...
def do_and_form(vals, env):
    ...
    if(len(vals)==0):
        ...
        return vals[0]
    elif scheme_true(scheme_eval(vals[0], Frame(env))):
...
def check_formals(formals):
    ...
    have_seen=Pair(nil,nil)
    for symbol in formals:
        if(not scheme_symbolp(symbol)):
            raise SchemeError
        if(symbol in have_seen):
            raise SchemeError
        else:
            have_seen=Pair(have_seen,Pair(symbol, nil))
