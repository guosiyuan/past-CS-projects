...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if (len(formals) != len(vals)):
            raise SchemeError("Formals and Values not matching")
...
def do_mu_form(vals):
    ...
    "*** YOUR CODE HERE ***"
    if (len(vals) > 2):
        return MuProcedure(vals[0], Pair('begin', vals.second))
    return MuProcedure(vals[0], vals[1])
...
def do_let_form(vals, env):
    ...
    for binding in bindings:
        ...
        if not scheme_symbolp(binding[0]):
            raise SchemeError("Binding name {0} not valid".format(str(binding[0])))
...
def check_formals(formals):
    ...
    for formal in formals:
        if not scheme_symbolp(formal):
            ...
            raise SchemeError("Repetitive formals")
        else:
            read.append(formal)
