...
def do_and_form(vals, env):
    ...
    for k in range(len(vals)-1):
        ...
        current=scheme_eval(vals[k],env)
        if(scheme_false(current)):
            return False

  
    last_item=scheme_eval(vals[len(vals)-1],env)
...
def do_or_form(vals, env):
    ...
    for k in range(len(vals)-1):
        ...
        if(scheme_true(v)):
            return v
    last_item=vals[len(vals)-1]
    scheme_eval(last_item,env)
    if(scheme_true(last_item)):
        return quote(last_item)
    return False
